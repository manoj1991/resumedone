...
const Sidebar = ({data}) => {
    ...
    const handleAddBoard = () => {
        ...
        const newBoard = { title: 'New Board', class: 'new-board-icon' };
    
        // Creating a copy of the current boards array and appending the new board
        const updatedBoards = [...data.boards, newBoard];
    ...
    return (
        <nav id="sidebar" className={`sidebar ${sidebarOpen ? '' : 'hide'} position-relative`}>
            ...
            <div className={`sidebar-footer flex-display ${sidebarOpen ? '' : hideClass}`}>
                <Image src={'/assets/img/clara.png'} width={40} height={40} className='rounded-circle' alt='profile setting'/> <span>Carla</span> <GearFill   width={20} height={20} className='setup-icon ms-auto'/>
            </div>
